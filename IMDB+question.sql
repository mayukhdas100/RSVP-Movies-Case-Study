...
WHERE 
	r.avg_rating > 8
    AND g.genre IN (cte_top_genre.genre)
GROUP BY n.name
ORDER BY movie_count DESC
)
SELECT
...
INNER JOIN genre g
ON m.id = g.movie_id
WHERE genre IN(SELECT genre FROM cte_top3_genre)
)
SELECT *
FROM cte_top5_movie
WHERE movie_rank<=5;
...
cte_top_director_summary AS
(
SELECT *,
Datediff(next_date_published, date_published) AS date_difference
FROM cte_date_summary
)
SELECT 
